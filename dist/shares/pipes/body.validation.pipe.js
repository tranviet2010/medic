"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.BodyValidationPipe = void 0;
const common_1 = require("@nestjs/common");
class BodyValidationPipe extends common_1.ValidationPipe {
    constructor() {
        super({
            transform: true,
            transformOptions: { enableImplicitConversion: true },
            skipMissingProperties: false,
            exceptionFactory: (errs) => {
                return new common_1.HttpException({
                    message: `Validation errors on these fields: ${this.getMessageFromErrs(errs)}`,
                    code: 'USER_00400',
                    statusCode: 400,
                    messageDetail: this.getPropertyAndContraints(errs),
                }, common_1.HttpStatus.BAD_REQUEST);
            },
        });
    }
    getMessageFromErrs(errs, parent = null) {
        return errs
            .map((e) => {
            const current = parent ? `${parent}.${e.property}` : `${e.property}`;
            if (e.children.length > 0)
                return `${this.getMessageFromErrs(e.children, current)}`;
            else
                return current;
        })
            .join(', ');
    }
    getPropertyAndContraints(errs) {
        const details = [];
        errs.forEach((e) => {
            if (e.children.length > 0) {
                this.getPropertyAndContraints(e.children).forEach((e) => details.push(e));
            }
            else {
                details.push({
                    property: e.property,
                    contraints: Object.values(e.constraints),
                });
            }
        });
        return details;
    }
}
exports.BodyValidationPipe = BodyValidationPipe;
//# sourceMappingURL=body.validation.pipe.js.map